...
def detect_faces_image(image: str) -> dict:
    '''
...
# @xray_recorder.capture('detect_faces')
def detect_faces(bucket: str, name: str) -> dict:
    '''
    Invoke the rekognition:detect_faces method.
    :param image: The utf8(base64( image-bytes ))
    :rtype: The response from detect_faces method.
    '''
    assert bucket is not None and name is not None, "detect_faces missing image argument."
    response = client.detect_faces(
        Attributes=['ALL'],
        Image={
            "S3Object": {
...
def function_main(event: Mapping[str, Any], _=None):
    ...
    if event.get('Image', None) != None:
        response = detect_faces_image(event['Image'])
    else:
        response = detect_faces(event['Bucket'], event['Name'])

    '''
  Confirm the face is usable...
  '''
    valid_faces = [face for face in response['FaceDetails']
