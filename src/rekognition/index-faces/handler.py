...
def get_partition_count() -> int:
    ...
    try:
        parameter_name = '/riv/{}/rekognition/partition-count'.format(
            riv_stack_name)
        ...
        logger.error(
            'Unable to get_partition_count() - run src/rekognition/setup.')
        raise error
    finally:
  ...
  '''
    if not len(response['FaceRecords']) == 1:
        raise NotImplementedError(
            'This sample only supports one indexed face.')
